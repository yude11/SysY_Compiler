...
std::vector<FunctionSymbol> builtin_functions = {
    ...
    {"getarray", nullptr, TypeInfo(BaseType::INT, {}), {TypeInfo(BaseType::INT, {{TypeInfo::TypeKind::POINTER, 0}})}},
    // void putint(int x)
    {"putint", nullptr, TypeInfo(BaseType::VOID, {}), {TypeInfo(BaseType::INT, {})}},
    // void putch(int x)
    {"putch", nullptr, TypeInfo(BaseType::VOID, {}), {TypeInfo(BaseType::INT, {})}},
    // void putarray(int n, int *arr)
    {"putarray",
     nullptr,
     TypeInfo(BaseType::VOID, {}),
...
IRgenerator::IRgenerator()
    : program(std::make_unique<Program>()),
      ...
      name_manager(std::make_unique<IRNameManager>()) {}

IRgenerator::~IRgenerator() = default;

TypeInfo IRgenerator::TypeInfoModelToTypeInfo(TypeInfoModel *type_model) const {
  std::vector<std::pair<TypeInfo::TypeKind, int>> modifiers;

  // SCALAR: 没有修饰符
  if (type_model->kind == TypeInfo::TypeKind::SCALAR) {
    return {type_model->type, modifiers};
  }

  // POINTER: 添加指针修饰符
  if (type_model->kind == TypeInfo::TypeKind::POINTER) {
    modifiers.emplace_back(TypeInfo::TypeKind::POINTER, 0);
  }
  ...
  if (type_model->dims) {
    for (auto &dim_ast : *type_model->dims) {
      ...
      modifiers.emplace_back(TypeInfo::TypeKind::ARRAY, dim_size);
    }
  }
...
template <typename T>
void IRgenerator::ParseArrayInitVals(T *ast, std::vector<std::shared_ptr<Value>> &init_vals,
                                     const std::vector<int> &dims, int level) {
  ExpComputer computer(symbol_table.get());
...
void IRgenerator::Visit(CompUnitAST *ast) {
  ...
  for (auto &func : builtin_functions) {
    auto func_decl = std::make_unique<Function_Decl>(func.return_type.base, func.return_type.modifiers, func.name);
  ...
  for (auto &func_def_or_decl : ast->func_defs_or_decls) {
    if (dynamic_cast<FuncDefAST *>(func_def_or_decl.get()) != nullptr) {
      ...
      // 将函数加入到符号表中
      symbol_table->Insert(def->ident, nullptr, TypeInfoModelToTypeInfo(def->func_type.get()), params_type);
      // 处理函数
      func_def_or_decl->Accept(this);
      program->functions.push_back(std::move(current_func));
    } else if (dynamic_cast<VarDeclAST *>(func_def_or_decl.get()) != nullptr) {
      ...
      for (auto &def : *(decl->var_def_list)) {
        ...
        switch (var_def->type) {
          case 0: {
            ...
            init_vals.push_back(std::make_shared<Value_INTEGER>(0));
            auto global_alloc =
                std::make_shared<Value_GLOBOL_ALLOC>(name_manager->AllocateNamed(var_def->ident), init_vals, var_type);
          ...
          case 1: {
            ...
            init_vals.push_back(std::make_shared<Value_INTEGER>(global_var_computer.Evaluate(exps->exp.get())));
            auto global_alloc =
                std::make_shared<Value_GLOBOL_ALLOC>(name_manager->AllocateNamed(var_def->ident), init_vals, var_type);
            program->values.push_back(global_alloc);
            symbol_table->Insert(var_def->ident, global_alloc, var_type, false);
          ...
          case 2: {
            ...
            // 使用默认值初始化数组
            std::vector<std::shared_ptr<Value>> init_vals(size, std::make_shared<Value_INTEGER>(0));
            // 构建数组类型
            TypeInfo array_type = var_type;
            for (int d : dims) {
              array_type.modifiers.emplace_back(TypeInfo::TypeKind::ARRAY, d);
            }
            // 插入符号表和IR
            auto global_alloc = std::make_shared<Value_GLOBOL_ALLOC>(name_manager->AllocateNamed(var_def->ident),
                                                                     init_vals, array_type);
            program->values.push_back(global_alloc);
    ...
    } else if (dynamic_cast<ConstDeclAST *>(func_def_or_decl.get()) != nullptr) {
      ...
      for (const auto &def : *(decl->const_def_list)) {
        ...
        switch (const_def->type) {
          case 0: {
            ...
            auto const_value = std::make_shared<Value_INTEGER>(global_var_computer.Evaluate(exps->exp.get()));
          ...
          case 1: {
            ...
            // 2. 将数组加入符号表
            auto global_alloc = std::make_shared<Value_GLOBOL_ALLOC>(name_manager->AllocateNamed(const_def->ident),
                                                                     init_vals, array_type);
            program->values.push_back(global_alloc);
...
void IRgenerator::Visit(FuncDefAST *ast) {
  ...
  symbol_table->Push();
  if (ast->func_params) {
    for (const auto &param : *ast->func_params) {
      // 转换参数类型
      TypeInfo param_type = TypeInfoModelToTypeInfo(param->type.get());

      // 将函数参数加入符号表，函数参数作为一个值的引用
      auto param_ref = std::make_shared<Value_FUNC_ARG_REF>(name_manager->AllocateNamed(param->ident), param_type);
...
void IRgenerator::Visit(StmtAST *ast) {
  ...
  switch (ast->type) {
    ...
    case Stmt_Type::AST_STMT_ASSIGN: {
      ...
      if (lval->type == 0) {
        ...
        for (size_t i = 0; i < indices.size(); i++) {
          ...
          if (i < symbol->type.modifiers.size() && symbol->type.modifiers[i].first == TypeInfo::TypeKind::ARRAY) {
            auto get_elem_ptr =
                std::make_shared<Value_GET_ELEM_PTR>(name_manager->AllocateTemp(), current_ptr, indices[i], elem_size);
            ...
            auto get_ptr =
                std::make_shared<Value_GET_PTR>(name_manager->AllocateTemp(), load_ptr, indices[i], elem_size);
    ...
    case Stmt_Type::AST_STMT_IF_ELSE: {
      ...
      std::string else_name = if_else_stmt.else_stmt != nullptr ? name_manager->AllocateLabel("if_else") : end_name;
    ...
    case Stmt_Type::AST_STMT_BREAK: {
      std::string end_name =
          "while_end" + ((while_entry_stack.top() != 0) ? "_" + std::to_string(while_entry_stack.top()) : "");
      current_block->stmts.push_back(std::make_shared<Value_JUMP>(end_name));
      // 标记当前块已终止
      current_block->Terminate();
      break;
    ...
    case Stmt_Type::AST_STMT_CONTINUE: {
      std::string while_entry_name =
          "while_entry" + ((while_entry_stack.top() != 0) ? "_" + std::to_string(while_entry_stack.top()) : "");
  ...
  if (ast->type == 0) {
    ...
    if (lhs_val->type == Value_Type::KOOPA_RVT_INTEGER) {
      ...
      if (rhs_val->type == Value_Type::KOOPA_RVT_INTEGER) {
        ...
        auto neq = std::make_shared<Value_BINARY>(name_manager->AllocateTemp(), rhs_val,
                                                  std::make_shared<Value_INTEGER>(0), Binary_Op_Type::KOOPA_RBO_NOT_EQ);
    ...
    auto eq = std::make_shared<Value_BINARY>(name_manager->AllocateTemp(), lhs_val, std::make_shared<Value_INTEGER>(0),
    ...
    auto rhs_val = current_value_stack.top();
    current_value_stack.pop();
    auto neq = std::make_shared<Value_BINARY>(name_manager->AllocateTemp(), rhs_val, std::make_shared<Value_INTEGER>(0),
...
void IRgenerator::Visit(LAndExpAST *ast) {
  ...
  if (ast->type == 0) {
    ...
    auto then_name = name_manager->AllocateLabel("if_then");
    auto end_name = name_manager->AllocateLabel("if_end");
    auto ne = std::make_shared<Value_BINARY>(name_manager->AllocateTemp(), lhs_val, std::make_shared<Value_INTEGER>(0),
                                             Binary_Op_Type::KOOPA_RBO_NOT_EQ);
...
void IRgenerator::Visit(UnaryOpAST *ast) {
  ...
  switch (ast->op) {
    case Op_Type::AST_UNARY_OP_NEG: {
      // 将val实例加入到block
      auto val =
          std::make_shared<Value_BINARY>(name, std::make_shared<Value_INTEGER>(0), rhs, Binary_Op_Type::KOOPA_RBO_SUB);
      current_block->stmts.push_back(val);
      // 生成的val加入栈中
    ...
    case Op_Type::AST_UNARY_OP_NOT: {
      // 将val实例加入到block
      auto val =
          std::make_shared<Value_BINARY>(name, rhs, std::make_shared<Value_INTEGER>(0), Binary_Op_Type::KOOPA_RBO_EQ);
      current_block->stmts.push_back(val);
      // 生成的val加入栈
      current_value_stack.push(val);
      break;
...
void IRgenerator::Visit(BinaryOpAST *ast) {
  ...
  // 将val实例加入到block
  auto val = std::make_shared<Value_BINARY>(name, lhs, rhs, op_type);
  current_block->stmts.push_back(val);
...
void IRgenerator::Visit(LValAST *ast) {
  ...
  if (var_symbol->type.isScalar()) {
    ...
    for (size_t i = 0; i < indices.size(); i++) {
      ...
      // 根据类型选择指令：数组用 getelemptr，指针用 getptr
      if (i < var_symbol->type.modifiers.size() && var_symbol->type.modifiers[i].first == TypeInfo::TypeKind::ARRAY) {
    ...
    for (auto &param : *ast->params) {
      ...
      if (param_val->type_info.isPointer() && param_val->type_info.modifiers.size() >= 2) {
        ...
        if (param_val->type_info.modifiers[1].first == TypeInfo::TypeKind::ARRAY) {
          auto decay = std::make_shared<Value_GET_ELEM_PTR>(name_manager->AllocateTemp(), param_val,
                                                            std::make_shared<Value_INTEGER>(0), 4);
...
void IRgenerator::OutputIR(const std::string &output) const {
  IROutputer out(output);
  program->Accept(&out);
}

IROutputer::IROutputer() = default;

IROutputer::IROutputer(const std::string &output) : fs(output, std::ios::out) {}

IROutputer::~IROutputer() = default;

void IROutputer::Visit(Value_INTEGER *integer) { fs << integer->val; }
...
void IROutputer::Visit(Value_FUNC_ARG_REF *ref) { fs << ref->name; }
...
void IROutputer::Visit(Value_LOAD *load) { fs << "  " << load->name << " = load " << load->src->name << std::endl; }
...
// 辅助函数：递归输出多维数组初始化值
void IROutputer::OutputArrayInit(const std::vector<std::shared_ptr<Value>> &init, const std::vector<int> &dims,
                                 int level, std::fstream &fs, int &index) {
  fs << "{";
...
void IROutputer::Visit(Value_GET_ELEM_PTR *get_elem_ptr) {
  fs << "  " << get_elem_ptr->name << " = getelemptr " << get_elem_ptr->base->name << ", " << get_elem_ptr->index->name
     << std::endl;
}
...
ExpComputer::~ExpComputer() = default;
...
void ExpComputer::Visit(LValAST *ast) {
  auto *symbol = symbol_table->FindSymbol(ast->ident);
  auto *var_symbol = static_cast<VariableSymbol *>(symbol);
  if (symbol != nullptr && var_symbol->is_const) {
...
void ExpComputer::Visit(NumberAST *ast) { val_stack->push(ast->num); }

void ExpComputer::Visit(ExpAST *ast) { ast->l_or_exp->Accept(this); }

void ExpComputer::Visit(PrimaryExpAST *ast) { ast->mem->Accept(this); }
...
void ExpComputer::Visit(LOrExpAST *ast) {
  if (ast->type == 0) {
    ...
    val_stack->pop();
    val_stack->push((lhs != 0) || (rhs != 0) ? 1 : 0);
  }
}
